...
A tool that populates each GoodData Cloud metadata facts/attributes/metrics/visuals/dashboards with the help of LLM. The result could be reviewed and pushed to the workspace. It keeps the generation history, so when triggered again, it will only populate the new entities.
